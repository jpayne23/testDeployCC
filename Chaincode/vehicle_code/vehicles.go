...
func (t *SimpleChaincode) add_data(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	for i:= 0; i < len(args); i++ {
		err := json.Unmarshal([]byte(args[i]), &ecert)
															if err != nil { return nil , errors.New(args[i] + " was invalid. Should be JSON with fields identity and eCert. Error: "+err.Error())}
...
func (t *SimpleChaincode) add_ecert(stub *shim.ChaincodeStub, name string, ecert string) (string, error) {
	...
	if err != nil {
		return "", errors.New("Insert eCert to table failed. Error: " + err.Error())
	}
	if !ok {
